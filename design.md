...
- **Chinese Typography (CJK)**: For Chinese content, use `PingFang SC`, `Hiragino Sans GB`, or `Microsoft YaHei`. Line-height should be more generous (1.6 to 1.8) to prevent visual density. Text should be justified with `text-justify: inter-character` for a clean, printed-book look.
...
5. The navigation glass effect (translucent dark + blur) is non-negotiable — it defines the App's aesthetic.
